...
function path_out=planRRT_RT(wpp_start, wpp_end, map, segmentLength, iterations)
...
function [new_tree,flag] = extendTree(tree,end_node,segmentLength,map,pd,chi)
  ...
  % update plot
  if size(tree,1)>1,
    X = [new_node(1), tree(new_node(6),1)];
    Y = [new_node(2), tree(new_node(6),2)];   
    Z = [new_node(3), tree(new_node(6),3)];            
    plot3(Y,X,-Z,'g') 
    drawnow
  end
  ...
  if ( (norm(new_node(1:3)-end_node(1:3))<segmentLength )...
    ...
    X = [new_node(1), end_node(1)];
    Y = [new_node(2), end_node(2)];   
    Z = [new_node(3), end_node(3)];            
    plot3(Y,X,-Z,'g') 
    drawnow
  else
    flag = 0;
