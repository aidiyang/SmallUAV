...
function path_out=planRRT(wpp_start, wpp_end, map, segmentLength, iterations)
...
%         for count = 1:(iterations-1),
    ...
    time_minpath = toc; % check timer
    
    % display results
    plotmap(map,path,path_out,tree);
    disp(['time to find minimum path:  ', num2str(time_minpath), ' sec']);
